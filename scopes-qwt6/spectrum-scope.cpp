...
spectrumScope::spectrumScope(QwtPlot* plot, int16_t displaySize) : virtualScope(plot, displaySize) {
...
#else
  grid->setMajorPen(QPen(Qt::white, 0, Qt::NoPen));
#endif
...
#else
  grid->setMinorPen(QPen(Qt::white, 0, Qt::NoPen));
#endif
  ...
  ourBrush = new QBrush(Qt::yellow);
 // ourBrush->setStyle(Qt::Dense3Pattern);
  ourBrush->setStyle(Qt::SolidPattern);
  ...
  QwtPickerClickPointMachine* lpickerMachine = new QwtPickerClickPointMachine();
  lm_picker->setStateMachine(lpickerMachine);
  lm_picker->setMousePattern(QwtPlotPicker::MouseSelect1, Qt::LeftButton);
  connect(lm_picker, SIGNAL(selected(const QPointF&)), this, SLOT(leftMouseClick(const QPointF&)));

  rm_picker = new QwtPlotPicker(plot->canvas());
  QwtPickerMachine* rpickerMachine = new QwtPickerClickPointMachine();
  rm_picker->setStateMachine(rpickerMachine);
  rm_picker->setMousePattern(QwtPlotPicker::MouseSelect1, Qt::RightButton);
  connect(rm_picker, SIGNAL(selected(const QPointF&)), this, SLOT(rightMouseClick(const QPointF&)));
...
void spectrumScope::leftMouseClick(const QPointF& point) { clickedwithLeft((int)(point.x()) - IndexforMarker); }

void spectrumScope::rightMouseClick(const QPointF& point) { clickedwithRight((int)(point.x()) - IndexforMarker); }

void spectrumScope::display(double* X_axis, double* Y1_value, double amp, int32_t markerValue, int32_t theBin) {
  int16_t i;
  double binVal = 0;
  ...
  plotgrid->setAxisScale(QwtPlot::yLeft, get_db(0), get_db(0) + amp);
...
float spectrumScope::get_db(float x) { return 20 * log10((x + 1) / (float)(normalizer)); }
