...
static inline float get_db(float x) { return 20 * log10((x + 1) / (float)(64)); }
//
audioScope::audioScope(QwtPlot* plotGrid, int16_t displaySize, int32_t sampleRate) {
  int16_t i;
  double temp;
  ...
  for (i = 0; i < spectrumSize; i++)
    Window[i] = 0.42 - 0.5 * cos((2.0 * M_PI * i) / (spectrumSize - 1)) + 0.08 * cos((4.0 * M_PI * i) / (spectrumSize - 1));

  temp = (double)sampleRate / 200 / displaySize;
  for (i = 0; i < displaySize / 2; i++)
    X_axis[i] = (double)((i) * (double)temp);
...
void audioScope::setRate(int newRate) {
  ...
  temp = (double)sampleRate / 200 / displaySize;
  for (i = 0; i < displaySize / 2; i++)
    X_axis[i] = (double)((i) * (double)sampleRate / displaySize);
}
...
void audioScope::addElement(std::complex<float> x) {
  ...
  for (i = 0; i < displaySize / 2; i++) {
    displayBuffer[i] = 0;
    for (j = 0; j < 4; j++) {
      displayBuffer[i] += 0.25 * abs(spectrumBuffer[4 * i + j]);
    }
  }
...
#define FREQ 3
  ...
  plotGrid->setAxisScale(QwtPlot::xBottom, (double)X_axis[0], X_axis[displaySize / 2 - 1]);
  plotGrid->enableAxis(QwtPlot::xBottom);
  plotGrid->setAxisScale(QwtPlot::yLeft, get_db(0), get_db(0) + 50);
