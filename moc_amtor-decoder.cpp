/****************************************************************************
** Meta object code from reading C++ file 'amtor-decoder.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.15.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include <memory>
#include "decoders/amtor-decoder/amtor-decoder.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'amtor-decoder.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.15.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_amtorDecoder_t {
    QByteArrayData data[6];
    char stringdata0[81];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_amtorDecoder_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_amtorDecoder_t qt_meta_stringdata_amtorDecoder = {
    {
QT_MOC_LITERAL(0, 0, 12), // "amtorDecoder"
QT_MOC_LITERAL(1, 13, 14), // "set_amtorAfcon"
QT_MOC_LITERAL(2, 28, 0), // ""
QT_MOC_LITERAL(3, 29, 16), // "set_amtorReverse"
QT_MOC_LITERAL(4, 46, 17), // "set_amtorFecError"
QT_MOC_LITERAL(5, 64, 16) // "set_amtorMessage"

    },
    "amtorDecoder\0set_amtorAfcon\0\0"
    "set_amtorReverse\0set_amtorFecError\0"
    "set_amtorMessage"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_amtorDecoder[] = {

 // content:
       8,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    1,   34,    2, 0x08 /* Private */,
       3,    1,   37,    2, 0x08 /* Private */,
       4,    1,   40,    2, 0x08 /* Private */,
       5,    1,   43,    2, 0x08 /* Private */,

 // slots: parameters
    QMetaType::Void, QMetaType::QString,    2,
    QMetaType::Void, QMetaType::QString,    2,
    QMetaType::Void, QMetaType::QString,    2,
    QMetaType::Void, QMetaType::QString,    2,

       0        // eod
};

void amtorDecoder::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto *_t = static_cast<amtorDecoder *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->set_amtorAfcon((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 1: _t->set_amtorReverse((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 2: _t->set_amtorFecError((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 3: _t->set_amtorMessage((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        default: ;
        }
    }
}

QT_INIT_METAOBJECT const QMetaObject amtorDecoder::staticMetaObject = { {
    QMetaObject::SuperData::link<virtualDecoder::staticMetaObject>(),
    qt_meta_stringdata_amtorDecoder.data,
    qt_meta_data_amtorDecoder,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *amtorDecoder::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *amtorDecoder::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_amtorDecoder.stringdata0))
        return static_cast<void*>(this);
    if (!strcmp(_clname, "Ui_amtorwidget"))
        return static_cast< Ui_amtorwidget*>(this);
    return virtualDecoder::qt_metacast(_clname);
}

int amtorDecoder::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = virtualDecoder::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
