...
int32_t decimator::inSize(void) { return inputLimit; }

int32_t decimator::outSize(void) { return outputLimit; }
...
bool decimator::add(std::complex<float> val, std::complex<float>* b) {
  ...
  for (j = 0; j < framesOut; j++)
    b[j] = std::complex<float>(10 * outBuffer[2 * j], 10 * outBuffer[2 * j + 1]);
  return true;
}
