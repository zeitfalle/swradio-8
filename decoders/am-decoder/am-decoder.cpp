...
amDecoder::amDecoder(int32_t inRate, RingBuffer<DSPCOMPLEX>* buffer, QSettings* settings) : virtualDecoder(inRate, buffer) {
  this->inRate = inRate;
  (void)settings;
  audioOut = buffer;
  myFrame = new QFrame;
  ...
  connect(adaptiveFilterButton, SIGNAL(clicked(void)), this, SLOT(set_adaptiveFilter(void)));
  connect(adaptiveFilterSlider, SIGNAL(valueChanged(int)), this, SLOT(set_adaptiveFiltersize(int)));
  connect(lowpassFilterslider, SIGNAL(valueChanged(int)), this, SLOT(set_lowpassFilter(int)));
  adaptiveFiltering = false;
...
void amDecoder::process(std::complex<float> z) {
  ...
  std::complex<float> t = lowpassFilter->Pass(std::complex<float>(res, res));
  ...
  if (counter > inRate / 10) {
    setDetectorMarker(0);
    counter = 0;
    emit audioAvailable(audioOut->GetRingBufferReadAvailable() / 10, inRate);
  }
}
...
void amDecoder::set_lowpassFilter(int v) {
  ...
  delete lowpassFilter;
  lowpassFilter = new LowPassFIR(5, float(v) * inRate / 101, inRate);
}
