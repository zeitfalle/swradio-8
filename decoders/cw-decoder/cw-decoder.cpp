...
cwDecoder::cwDecoder(int32_t inRate, RingBuffer<std::complex<float>>* buffer, QSettings* s)
    : virtualDecoder(inRate, buffer), inputBuffer(inRate), inputConverter(inRate, 2000), localShifter(10 * 2000) {
  theRate = inRate;
  (void)s;
  myFrame = new QFrame;
  setupUi(myFrame);
...
void cwDecoder::processSample(std::complex<float> z) {
  ...
  static std::complex<float> outV[2000];
  ...
  if (theFilter->Pass(s, &res)) {
    ...
    if (fillP >= workingRate / 4) {
      for (i = 0; i < screenwidth; i++)
        y_values[i] = abs(outV[(screenwidth / 2 + i) % screenwidth]);
...
int cwDecoder::offset(std::complex<float>* v) {
  ...
  if (supermax / MAX_SIZE > 3 * avg)
    return superIndex > screenwidth - 100 ? superIndex - screenwidth : superIndex;
  else
    return -100;
}
=======
