...
cwDecoder::cwDecoder(int32_t inRate, RingBuffer<std::complex<float>>* buffer, QSettings* s)
    : virtualDecoder(inRate, buffer), inputBuffer(inRate), inputConverter(inRate, 2000), localShifter(2000) {
  theRate = inRate;
  this->cwSettings = s;
  myFrame = new QFrame;
  setupUi(myFrame);
  ...
  cwViewer = new waterfallScope(cwScope, screenwidth, 15);

  newFFT = new slidingFFT(screenwidth, 0, screenwidth - 1);
  afcFFT = new slidingFFT(screenwidth, 0, screenwidth - 1);
  connect(cwViewer, SIGNAL(clickedwithLeft(int)), this, SLOT(handleClick(int)));
  ...
  connect(cw_rangeSetter, SIGNAL(valueChanged(int)), this, SLOT(set_searchRange(int)));
}
...
void cwDecoder::setup_cwDecoder(int32_t rate) {
  ...
  connect(SquelchLevel, SIGNAL(valueChanged(int)), this, SLOT(cw_setSquelchValue(int)));
  connect(WPM, SIGNAL(valueChanged(int)), this, SLOT(cw_setWordsperMinute(int)));
  connect(FilterDegree, SIGNAL(valueChanged(int)), this, SLOT(cw_setFilterDegree(int)));
  connect(Tracker, SIGNAL(clicked(void)), this, SLOT(cw_setTracking(void)));
...
void cwDecoder::cw_setFilterDegree(int n) {
  ...
  cw_BandPassFilter = new bandpassFIR(2 * cwFilterDegree + 1, cw_IF - 500, cw_IF + 500, workingRate);
}
...
void cwDecoder::processSample(std::complex<float> z) {
  ...
  if (fillP >= workingRate / 10) {
    for (i = 0; i < screenwidth; i++)
      y_values[i] = 0.6 * y_values[i] + 0.4 * abs(outV[(screenwidth / 2 + i) % screenwidth]);
    cwViewer->display(x_axis, y_values, amplitudeSlider->value(), 0, 0);
    fillP = 0;
    newFFT->reset();
  }
  ...
  switch (cwState) {
  case MODE_IDLE:
    if ((value > 0.67 * agc_peak) && (value > SquelchValue * noiseLevel)) {
      /*
...
int32_t cwDecoder::newThreshold(int32_t prev_element, int32_t curr_element) {
  ...
  if ((prev_element > 0) && (curr_element > 0)) {
    if ((curr_element > 2 * prev_element) && (curr_element < 4 * prev_element))
      return getMeanofDotDash(prev_element, curr_element);
    /*
     * 	check for dash dot sequence
     */
    if ((prev_element > 2 * curr_element) && (prev_element < 4 * curr_element))
...
void cwDecoder::printChar(char a, char er) {
  ...
  case 0:
    ...
    break;
  case 1:
    printf("\033[01;42m%c\033[m", a);
    break;
  case 2:
    printf("\033[01;41m%c\033[m", a);
    break;
  case 3:
    printf("\033[01;43m%c\033[m", a);
    break;
  case 4:
  case 5:
  case 6:
  case 7:
    printf("\033[2J\033[H<BRK>\n");
    break;
  default:
    break;
  }
...
const char* const codetable[] = {"A._",    "B_...",  "C_._.",  "D_..",   "E.",     "F.._.",  "G__.",    "H....",   "I..",     "J.___",
                                 "K_._",   "L._..",  "M__",    "N_.",    "O___",   "P.__.",  "Q__._",   "R._.",    "S...",    "T_",
                                 "U.._",   "V..._",  "W.__",   "X_.._",  "Y_.__",  "Z__..",  "0_____",  "1.____",  "2..___",  "3...__",
                                 "4...._", "5.....", "6_....", "7__...", "8___..", "9____.", ".._._._", ",__..__", "?..__..", "~"};
...
void cwDecoder::cw_showdotLength(int l) { dotLengthdisplay->display(l); }

void cwDecoder::cw_showspaceLength(int l) { spaceLengthdisplay->display(l); }

void cwDecoder::cw_showagcpeak(int l) { agcPeakdisplay->display(l); }

void cwDecoder::cw_shownoiseLevel(int l) { noiseLeveldisplay->display(l); }

void cwDecoder::cw_showspeed(int l) { actualWPM->display(l); }
...
void cwDecoder::set_searchRange(int n) { cw_searchRange = n; }
