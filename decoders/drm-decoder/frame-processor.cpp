...
frameProcessor::frameProcessor(drmDecoder* mr, RingBuffer<std::complex<float>>* buffer, RingBuffer<std::complex<float>>* iqBuffer,
                               RingBuffer<std::complex<float>>* eqBuffer, RingBuffer<std::complex<float>>* audioBuffer, int32_t sampleRate, int16_t nSymbols,
                               int8_t windowDepth, int8_t qam64Roulette)
    : my_Reader(buffer, 4 * 16384, mr), my_backendController(mr, iqBuffer, audioBuffer, qam64Roulette), theState(mr, 1, 3) {
  this->theDecoder = mr;
  this->buffer = buffer;
  ...
  connect(this, SIGNAL(setTimeSync(bool)), theDecoder, SLOT(executeTimeSync(bool)));
  connect(this, SIGNAL(setFACSync(bool)), theDecoder, SLOT(executeFACSync(bool)));
  connect(this, SIGNAL(setSDCSync(bool)), theDecoder, SLOT(executeSDCSync(bool)));
  connect(this, SIGNAL(show_Mode(int)), theDecoder, SLOT(execute_showMode(int)));
  connect(this, SIGNAL(show_Spectrum(int)), theDecoder, SLOT(execute_showSpectrum(int)));
  connect(this, SIGNAL(show_services(int, int)), theDecoder, SLOT(show_channels(int, int)));
...
void frameProcessor::run(void) {
  ...
  restart:
    ...
    fprintf(stderr, "found: Mode = %d, time_offset = %d, sampleoff = %f freqoff = %f\n", modeInf.Mode, modeInf.timeOffset_integer, modeInf.sampleRate_offset,
    ...
    int nrCarriers = Kmax(modeInf.Mode, modeInf.Spectrum) - Kmin(modeInf.Mode, modeInf.Spectrum) + 1;
    ...
    equalizer_1 my_Equalizer(theDecoder, modeInf.Mode, modeInf.Spectrum, windowDepth, eqBuffer);
    wordCollector my_wordCollector(theDecoder, &my_Reader, &modeInf, sampleRate);
    ...
    for (int symbol = 0; symbol < nrSymbols; symbol++) {
      my_wordCollector.getWord(inbank.element(symbol), modeInf.freqOffset_integer, modeInf.timeOffset_fractional, modeInf.freqOffset_fract);
    ...
    while (true) {
      my_wordCollector.getWord(inbank.element(lc), modeInf.freqOffset_integer, modeInf.timeOffset_fractional, modeInf.freqOffset_fract);
    ...
    for (symbol_no = 0; symbol_no < nrSymbols; symbol_no++)
      (void)my_Equalizer.equalize(inbank.element((lc + symbol_no) % nrSymbols), symbol_no, &outbank, &modeInf.timeOffset_fractional, &deltaFreqOffset,
                                  &sampleclockOffset);
    ...
    frameReady = false;
    while (!frameReady) {
      my_wordCollector.getWord(inbank.element(lc), modeInf.freqOffset_integer, false, modeInf.timeOffset_fractional, deltaFreqOffset, sampleclockOffset);
      frameReady = my_Equalizer.equalize(inbank.element(lc), symbol_no, &outbank, &modeInf.timeOffset_fractional, &deltaFreqOffset, &sampleclockOffset);
    ...
    inSync = my_facProcessor.processFAC(my_Equalizer.getMeanEnergy(), my_Equalizer.getChannels(), &outbank, &theState);
    //	one test:
    ...
    sdcProcessor my_sdcProcessor(theDecoder, &modeInf, &sdcTable, &theState);
    ...
    while (true) {
      ...
      if (isFirstFrame(&theState)) {
        ...
        setSDCSync(sdcOK);
        if (sdcOK) {
          show_services(getnrAudio(&theState), getnrData(&theState));
          blockCount = 0;
        }
        //
      ...
      for (int i = 0; !frameReady && (i < nrSymbols); i++) {
        my_wordCollector.getWord(inbank.element((lc + i) % nrSymbols),
                                 modeInf.freqOffset_integer, // initial value
                                 firstTime, modeInf.timeOffset_fractional,
                                 deltaFreqOffset,  // tracking value
                                 sampleclockOffset // tracking value
        );
        firstTime = false;

        frameReady = my_Equalizer.equalize(inbank.element((lc + i) % nrSymbols), (symbol_no + i) % nrSymbols, &outbank, &modeInf.timeOffset_fractional,
                                           &deltaFreqOffset, &sampleclockOffset);
      }

      //	OK, let us check the FAC
      bool success = my_facProcessor.processFAC(my_Equalizer.getMeanEnergy(), my_Equalizer.getChannels(), &outbank, &theState);
      if (success)
        missers = 0;
      else {
        setFACSync(false);
...
void frameProcessor::addtoSuperFrame(smodeInfo* m, int16_t blockno, myArray<theSignal>* outbank) {
  static int teller = 0;
  ...
  for (symbol = 0; symbol < symbolsperFrame(m->Mode); symbol++) {
    for (carrier = K_min; carrier <= K_max; carrier++)
      if (isDatacell(&modeInf, symbol, carrier, blockno)) {
        my_backendController.addtoMux(blockno, teller++, outbank->element(symbol)[carrier - K_min]);
      }
  }
...
bool frameProcessor::isDatacell(smodeInfo* m, int16_t symbol, int16_t carrier, int16_t blockno) {
  if (carrier == 0)
    return false;
...
int16_t frameProcessor::sdcCells(smodeInfo* m) {
  static int m1_table[] = {167, 190, 359, 405, 754, 846};
  static int m2_table[] = {130, 150, 282, 322, 588, 1500};
...
bool frameProcessor::isFACcell(smodeInfo* m, int16_t symbol, int16_t carrier) {
  int16_t i;
  ...
  for (i = 0; facTable[i].symbol != -1; i++) {
    if (facTable[i].symbol > symbol)
      return false;
    if ((facTable[i].symbol == symbol) && (facTable[i].carrier == carrier))
      return true;
  }
  return false;
}

bool frameProcessor::isSDCcell(smodeInfo* m, int16_t symbol, int16_t carrier) {
  if (carrier == 0)
    return false;
  if ((m->Mode == 1) && ((carrier == -1) || (carrier == 1)))
    return false;
...
void frameProcessor::set_sdcCells(smodeInfo* modeInf) {
  ...
  int carrier;
  int cnt = 0;
  for (carrier = Kmin(Mode, Spectrum); carrier <= Kmax(Mode, Spectrum); carrier++) {
...
void frameProcessor::frequencySync(drmDecoder* mr, Reader* my_Reader, smodeInfo* m) {
