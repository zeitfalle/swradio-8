...
struct testCells testCellsforModeA[] = {{18, 205}, {54, 836}, {72, 215}, {-1, -1}};

struct testCells testCellsforModeB[] = {{16, 331}, {48, 651}, {64, 555}, {-1, -1}};

struct testCells testCellsforModeC[] = {{11, 214}, {33, 392}, {44, 242}, {-1, -1}};

struct testCells testCellsforModeD[] = {{7, 788}, {21, 1014}, {28, 332}, {-1, -1}};
//	The frequency shifter is in steps of 0.01 Hz
freqSyncer::freqSyncer(Reader* b, smodeInfo* m, int32_t sampleRate, drmDecoder* mr) : theShifter(100 * sampleRate) {
  int16_t i;
  this->buffer = b;
  this->Mode = m->Mode;
  ...
  struct testCells* base = Mode == Mode_A ? testCellsforModeA : Mode == Mode_B ? testCellsforModeB : Mode == Mode_C ? testCellsforModeC : testCellsforModeD;
  ...
  fft_vector = (std::complex<float>*)fftwf_malloc(Tu_of(Mode) * sizeof(fftwf_complex));
  hetPlan =
      fftwf_plan_dft_1d(Tu_of(Mode), reinterpret_cast<fftwf_complex*>(fft_vector), reinterpret_cast<fftwf_complex*>(fft_vector), FFTW_FORWARD, FFTW_ESTIMATE);
  connect(this, SIGNAL(show_fineOffset(float)), mr, SLOT(show_fineOffset(float)));
  connect(this, SIGNAL(show_coarseOffset(float)), mr, SLOT(show_coarseOffset(float)));
  connect(this, SIGNAL(show_timeDelay(float)), mr, SLOT(show_timeDelay(float)));
  connect(this, SIGNAL(show_timeOffset(float)), mr, SLOT(show_timeOffset(float)));
  connect(this, SIGNAL(show_clockOffset(float)), mr, SLOT(show_clockOffset(float)));
  connect(this, SIGNAL(show_angle(float)), mr, SLOT(show_angle(float)));
}
...
bool freqSyncer::frequencySync(smodeInfo* m) {
  ...
  for (i = 0; i < N_symbols; i++) {
    int16_t time_offset_integer = getWord(buffer->data, buffer->bufSize, buffer->currentIndex + localIndex, i, 0, m->timeOffset_fractional);
  ...
  localIndex = 0;
  for (i = 0; i < N_symbols; i++) {
    int16_t time_offset_integer = getWord(buffer->data, buffer->bufSize, buffer->currentIndex + localIndex, i, m->freqOffset_integer, m->timeOffset_fractional);
    localIndex += Ts + time_offset_integer;
  }
  ...
  //	m -> Spectrum = 3;
  fprintf(stderr, "Spectrum is set to %d\n", m->Spectrum);
  return true;
}
...
int32_t freqSyncer::get_zeroBin(int16_t start) {
  ...
  for (j = start + 1; j < start + N_symbols; j++) {
    ...
    for (i = 0; i < Tu; i++) {
      std::complex<float> tmp1 = symbolBuffer[jmin1][i] * conj(symbolBuffer[jj][i]);
  ...
  for (i = -Tu / 10; i < Tu / 10; i++) {
    float sum = abs_sum[k_pilot1 + i] + abs_sum[k_pilot2 + i] + abs_sum[k_pilot3 + i];
    if (sum > highest) {
      dcOffset = i;
      highest = sum;
    }
...
float freqSyncer::get_spectrumOccupancy(uint8_t spectrum, int16_t baseBin) {
  int16_t i, j;
  ...
  for (i = 0; i < N_symbols; i++) {
    ...
    for (j = 0; j < 10; j++) {
      ...
      tmp3 += real(symbolBuffer[i][ind1] * conj(symbolBuffer[i][ind1]));
      tmp4 += real(symbolBuffer[i][ind2] * conj(symbolBuffer[i][ind2]));
    ...
    for (j = 0; j < 20; j++) {
      ...
      tmp5 += real(symbolBuffer[i][ind1] * conj(symbolBuffer[i][ind1]));
      tmp6 += real(symbolBuffer[i][ind2] * conj(symbolBuffer[i][ind2]));
  ...
  return energy_ratio_K_max_to_K_max_p4 + energy_ratio_K_min_to_K_min_p4;
}
//
...
int16_t freqSyncer::getWord(std::complex<float>* buffer, int32_t bufSize, int32_t theIndex, int16_t wordNumber, int intOffset, float offsetFractional) {
  std::complex<float> temp[Ts];
  int16_t i;
  uint32_t bufMask = bufSize - 1;
std:;
  complex<float> angle = std::complex<float>(0, 0);
  ...
  for (i = 0; i < Ts; i++) {
    ...
    temp[i] = cmul(een, 1 - offsetFractional) + cmul(twee, offsetFractional);
  }
  ...
  memcpy(symbolBuffer[wordNumber], &fft_vector[Tu / 2], Tu / 2 * sizeof(std::complex<float>));
  memcpy(&symbolBuffer[wordNumber][Tu / 2], fft_vector, Tu / 2 * sizeof(std::complex<float>));
  return 0;
}
