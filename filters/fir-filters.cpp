...
LowPassFIR::LowPassFIR(int16_t firsize, int32_t Fc, int32_t fs) : Basic_FIR(firsize) {
  sampleRate = fs;
  newKernel(Fc);
}
...
void LowPassFIR::newKernel(int32_t Fc) {
  ...
  for (i = 0; i < filterSize; i++) {
    ...
    //
    //	Blackman window
    tmp[i] *= (0.42 - 0.5 * cos(2 * M_PI * (float)i / filterSize) + 0.08 * cos(4 * M_PI * (float)i / filterSize));
...
DSPCOMPLEX* LowPassFIR::getKernel(void) { return filterKernel; }

//=====================================================================
// FIR HighPass

HighPassFIR::HighPassFIR(int16_t firsize, int32_t Fc, int32_t fs) : Basic_FIR(firsize) {
  sampleRate = fs;
  newKernel(Fc);
}
...
void HighPassFIR::newKernel(int32_t Fc) {
  ...
  for (i = 0; i < filterSize; i++) {
    ...
    tmp[i] *= (0.42 - 0.5 * cos(2 * M_PI * (float)i / (float)filterSize) + 0.08 * cos(4 * M_PI * (float)i / (float)filterSize));
 ...
 */
static float Blackman(float* v, int fsize, float f) {
  int i;
  float sum = 0;
  ...
  for (i = 0; i < fsize; i++) {
    ...
    v[i] *= (0.42 - 0.5 * cos(2 * M_PI * (float)i / (float)fsize) + 0.08 * cos(4 * M_PI * (float)i / (float)fsize));
...
static float* BandPassKernel(float* v, int16_t fsize, float Fcl, float Fch) {
  float sumA = 0.0;
  float sumB = 0.0;
  int16_t i;
  float tmp1[fsize];
...
BasicBandPass::BasicBandPass(int16_t firsize, int32_t low, int32_t high, int32_t rate) : Basic_FIR(firsize) {
  float t1[firsize];
  int16_t i;

  sampleRate = rate;
  (void)BandPassKernel(t1, firsize, (float)low / rate, (float)high / rate);
  for (i = 0; i < filterSize; i++)
...
DSPCOMPLEX* BasicBandPass::getKernel() { return filterKernel; }
 ...
 */
bandpassFIR::bandpassFIR(int16_t firSize, int32_t low, int32_t high, int32_t fs) : Basic_FIR(firSize) {
  sampleRate = fs;
  newKernel(low, high);
}
//
...
void bandpassFIR::newKernel(int32_t low, int32_t high) {
  ...
  for (i = 0; i < filterSize; i++) { // shifting
    ...
    filterKernel[i] = DSPCOMPLEX(tmp[i] * cos(v) / sum, tmp[i] * sin(v) / sum);
  }
}

bandpassFIR::~bandpassFIR() {}

DSPCOMPLEX* bandpassFIR::getKernel() { return filterKernel; }
...
decimatingFIR::decimatingFIR(int16_t firSize, int32_t low, int32_t fs, int16_t Dm) : Basic_FIR(firSize) {
...
decimatingFIR::decimatingFIR(int16_t firSize, int32_t low, int32_t high, int32_t fs, int16_t Dm) : Basic_FIR(firSize) {
...
void HilbertFilter::adjustFilter(float centre) {
  ...
  for (i = 0; i < firsize; i++) {
    ...
    cosKernel[i] = v1[i] * cos(omega * (i - ((float)firsize - 1) / (2.0 * rate)));
    sinKernel[i] = v1[i] * sin(omega * (i - ((float)firsize - 1) / (2.0 * rate)));
    Buffer[i] = 0;
  }
...
DSPCOMPLEX HilbertFilter::Pass(float a, float b) { return Pass(DSPCOMPLEX(a, b)); }
