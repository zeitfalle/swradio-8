...
fftFilter::fftFilter(int32_t size, int16_t degree) {
  ...
  for (i = 0; i < filterDegree; i++)
    blackman[i] = (0.42 - 0.5 * cos(2 * M_PI * (float)i / (float)filterDegree) + 0.08 * cos(4 * M_PI * (float)i / (float)filterDegree));
}
...
void fftFilter::setBand(int32_t low, int32_t high, int32_t rate) {
  ...
  for (i = 0; i < filterDegree; i++) {
    if (i == filterDegree / 2)
      ...
      tmp[i] = sin(2 * M_PI * lo * (i - filterDegree / 2)) / (i - filterDegree / 2);
    //
  ...
  for (i = 0; i < filterDegree; i++) { // shifting
    ...
    filterVector[i] = DSPCOMPLEX(tmp[i] * cos(v) / sum, tmp[i] * sin(v) / sum);
  }
...
void fftFilter::setLowPass(int32_t low, int32_t rate) {
  int32_t i;
  LowPassFIR* LowPass = new LowPassFIR((int16_t)filterDegree, low, rate);
...
float fftFilter::Pass(float x) {
  ...
  if (++inp >= NumofSamples) {
    ...
    for (j = 0; j < fftSize; j++) {
      ...
      FFT_C[j] = DSPCOMPLEX(real(FFT_C[j]) * 10, imag(FFT_C[j]) * 10);
    }
