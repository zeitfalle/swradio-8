...
static float sinhm1(float x) { return log(x + sqrt(x * x + 1)); }

static float coshm1(float x) { return log(x + sqrt(x * x - 1)); }
...
int guessOrderIC(int apass, int astop, int wpass, int wstop) { return guessOrderC(apass, astop, wpass, wstop); }

static float Bilineair(element* Transfer, int fs, int NumofQuad) {
  element* CurrentQuad;
  ...
  for (i = 0; i < NumofQuad; i++) {
    ...
    N0 = CurrentQuad->A0 * f4 + CurrentQuad->A1 * f2 + CurrentQuad->A2;
    N1 = 2 * (CurrentQuad->A2 - CurrentQuad->A0 * f4);
    N2 = CurrentQuad->A0 * f4 - CurrentQuad->A1 * f2 + CurrentQuad->A2;

    D0 = CurrentQuad->B0 * f4 + CurrentQuad->B1 * f2 + CurrentQuad->B2;
    D1 = 2 * (CurrentQuad->B2 - CurrentQuad->B0 * f4);
    D2 = CurrentQuad->B0 * f4 - CurrentQuad->B1 * f2 + CurrentQuad->B2;
...
float warpDtoA(int fd, int fs) { return 2.0 * fs * tan((2 * M_PI * fd) / (2 * fs)); }

int warpAtoD(float omega, int fs) { return (int)(2.0 * fs / tan(omega / (2 * fs))); }

float newButterworth(element* biQuads, int numofQuads, int order, int apass) {
  element* currentQuad;
  int16_t i;
  float Phim;
  float sigma, omega;
...
float newChebyshev(element* biQuads, int numofQuads, int order, int apass) {
  int16_t i;
  float sinhD, coshD;
  float Phim;
  float sigma, omega;
...
float newChebyshevI(element* biQuads, int numofQuads, int order, int apass) {
  int i;
  element* currentQuad;
  float Phim;
...
float newNormalized(element* Quads, int32_t numofQuads, int32_t order, int32_t apass, int8_t fType) {
  /*
   */
  ...
  else if (fType == S_BUTTERWORTH)
    return newButterworth(Quads, numofQuads, order, apass);
  else if (fType == S_INV_CHEBYSHEV)
 ...
 */
void cQuadratic(DSPCOMPLEX A, DSPCOMPLEX B, DSPCOMPLEX C, DSPCOMPLEX* D, DSPCOMPLEX* E) {
 ...
 */
float unnormalizeBP(element* temp, element* Transfer, float Wo, float BW, int numofbiQuads) {
...
LowPassIIR::LowPassIIR(int16_t apass, int16_t astop, int32_t fpass, int32_t fstop, int32_t fs) : Basic_IIR((guessOrderB(apass, astop, fpass, fstop) + 1) / 2) {
  ...
  gain = newNormalized(Quads, numofQuads, order, apass, S_BUTTERWORTH);
  //
...
LowPassIIR::LowPassIIR(int16_t order, int32_t fpass, int32_t fs, int16_t ftype) : Basic_IIR(((order + 1) & MAXORDER) / 2) {
...
HighPassIIR::HighPassIIR(int16_t order, int32_t fpass, int32_t fs, int16_t ftype) : Basic_IIR(((order + 1) & MAXORDER) / 2) {
...
BandPassIIR::BandPassIIR(int16_t order, int32_t flow, int32_t fhigh, int32_t fs, int16_t ftype) : Basic_IIR((order + 1) & MAXORDER) {
  float omegaL, omegaH;
