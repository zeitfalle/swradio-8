...
static inline bool isInfinite(double x) { return x == numeric_limits<float>::infinity(); }
//
static inline DSPCOMPLEX cmul(DSPCOMPLEX x, float y) { return DSPCOMPLEX(real(x) * y, imag(x) * y); }

static inline DSPCOMPLEX cdiv(DSPCOMPLEX x, float y) { return DSPCOMPLEX(real(x) / y, imag(x) / y); }

static inline float get_db(float x, int32_t y) { return 20 * log10((x + 1) / (float)(y)); }

static inline float PI_Constrain(float val) {
  if (0 <= val && val < 2 * M_PI)
    return val;
  if (val >= 2 * M_PI)
